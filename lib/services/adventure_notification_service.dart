...
class AdventureNotificationService {
  ...
  static const _messages = [
    ...
    '{enemy} yaklaşıyor! Kalan {steps} adımı tamamla.',
    'Ritmini koru! {enemy} karşısında {steps} adımın kaldı.',
    'Macera devam ediyor: {steps} adım daha at ve {enemy} gücünü kaybetsin!',
  ];
  ...
  static Future<void> scheduleAdventureReminders(
    ...
    final remainingSteps = adventure.remainingHealth(currentSteps);
    final remainingStages =
        (remainingSteps / AdventureQuest.stageStepTarget).ceil();
    final estimatedMinutes =
        (remainingSteps / AdventureQuest.briskWalkingStepsPerMinute).ceil() +
        remainingStages * AdventureQuest.syncGraceMinutes;
    final reminders = min(_notificationCount, max(1, estimatedMinutes ~/ 10));
    ...
    for (var i = 0; i < reminders; i++) {
      ...
      await _notifications.zonedSchedule(
        ...
        'Rush for Villains',
        body,
        tz.TZDateTime.now(tz.UTC).add(Duration(minutes: (i + 1) * 10)),
        details,
